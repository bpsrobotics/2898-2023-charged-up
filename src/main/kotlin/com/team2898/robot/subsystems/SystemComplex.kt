...
object SystemComplex: SubsystemBase() {
    ...
    val ballCount get() = run {var ct = 0; if(firstBall){ct+=1}; if(secondBall){ct+=1}; ct}
    fun Shoot(distance: Meters){
        ...
        if(abs(targetMotorSpeeds.first.value-currentMotorSpeeds.first.value) < Constants.SHOOTER_THRESHOLD && abs(targetMotorSpeeds.second.value-currentMotorSpeeds.second.value) < Constants.SHOOTER_THRESHOLD){
            forceShoot()
        }
    }
    fun forceShoot(){
